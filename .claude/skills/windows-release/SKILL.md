...
Ships `XinyunOpen-Bot-<version>-setup.exe` and its update feed to
[dongpen-max/xinyunopenbot-releases](https://github.com/dongpen-max/xinyunopenbot-releases).
...
| File | Purpose |
|---|---|
| `XinyunOpen-Bot-<version>-setup.exe` | the installer |
| `latest.yml` | **the update feed** — see step 4 |
| `XinyunOpen-Bot-<version>-setup.exe.blockmap` | differential updates |
| `XinyunOpen-Bot-<version>-x64.zip` | portable, not used by the updater |
...
- `app-update.yml` must point at `dongpen-max/xinyunopenbot-releases` and, while the
...
```powershell
Copy-Item release/XinyunOpen-Bot-<version>-setup.exe release/XinyunOpen-Bot-setup.exe
gh release upload v<version> --repo dongpen-max/xinyunopenbot-releases `
  release/XinyunOpen-Bot-<version>-setup.exe `
  release/XinyunOpen-Bot-setup.exe `
  release/XinyunOpen-Bot-<version>-setup.exe.blockmap `
  release/latest.yml
```

Both names are required, for different consumers:

- **`XinyunOpen-Bot-<version>-setup.exe`** is what `latest.yml` references by name and
  sha512. The auto-updater downloads exactly this.
- **`XinyunOpen-Bot-setup.exe`** is a byte-identical copy that gives the README's
  `/releases/latest/download/XinyunOpen-Bot-setup.exe` button a stable URL. This
  mirrors `XinyunOpen-Bot.dmg` sitting beside `XinyunOpen-Bot-<version>.dmg`.
