...
import { autoApprovedMcpTools, mcpStdioConfigs } from "../mcp.js";
...
const DENY_TIMEOUT_NOTE = "XinyunOpen Bot: nobody answered this permission request in time. Skip this action and finish what you can without it.";
        ...
        close() {
            for (const p of [...pending.values()]) {
                if (p.ask.kind === "question")
                    p.finish("answer", "XinyunOpen Bot: the turn is ending — wrap up.", "shutdown");
                else
                    p.finish("deny", "XinyunOpen Bot: the turn ended", "shutdown");
            }
            try {
                server.close();
            }
...
export function claudeEnvironment(instanceEnvironment, baseEnvironment = process.env) {
    const env = {
        ...baseEnvironment,
        ...instanceEnvironment,
        PATH: augmentedPath(),
    ...
    // A nested CLI must not inherit this session's identity (agentcal).
    delete env.CLAUDECODE;
    delete env.CLAUDE_CODE_ENTRYPOINT;
    // Subscription users get billed pay-as-you-go if a key leaks through, so
    // the key is only forwarded when a custom endpoint is configured — that
    // combination means "bring your own gateway", not a subscription login.
...
export const ClaudeDriver = {
    ...
    async create(input) {
        ...
        const sendTurn = async (turn) => {
            ...
            const env = claudeEnvironment(input.environment);
        ...
        const snapshot = async () => {
            ...
            // Either a subscription login, or a key aimed at a custom endpoint.
            const authenticated = existsSync(join(homedir(), ".claude", ".credentials.json")) ||
                Boolean(env.ANTHROPIC_BASE_URL && env.ANTHROPIC_API_KEY);
        ...
        return {
            ...
            adapter: {
                provider: DRIVER_KIND,
                capabilities: { sessionModelSwitch: "in-session", agentsMcp: true, mcpTools: true, computerMcp: true, computerMode: "mcp", sessionResume: true, streaming: true, coding: true },
                sendTurn,
