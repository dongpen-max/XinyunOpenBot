...
export function upsertMcpServer(cfg, input) {
    ...
    if (input.id === undefined) {
        let suffix = 2;
        const base = id;
        while (cfg.mcp?.servers?.[id])
            id = `${base.slice(0, 30 - String(suffix).length)}-${suffix++}`;
    }
    if (reservedIds.has(id))
        throw new Error(`MCP 服务 ID “${id}” 与内置工具冲突`);
    ...
    const allowedTools = Array.isArray(input.allowedTools)
        ? [...new Set(input.allowedTools.filter((name) => typeof name === "string" && Boolean(name.trim())).map((name) => name.trim()))].slice(0, 100)
    ...
    if (type === "apiKey" && header && !validHeader.test(header))
        throw new Error("API Key 请求头名称无效");
    ...
    const servers = { ...(cfg.mcp?.servers ?? {}), [id]: { name, url, enabled, ...(auth ? { auth } : {}), ...(allowedTools?.length ? { allowedTools } : {}) } };
    return { id, servers };
}
...
export async function probeMcpServer(server) {
    ...
    try {
        const initialized = await requestRemoteMcp(server, {
            ...
            id: 1,
            method: "initialize",
            params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "xinyunopen-bot", version: "0.1.30" } },
        }, session);
        ...
        return Array.isArray(result?.tools)
            ? result.tools.flatMap((tool) => typeof tool?.name === "string" ? [{ name: tool.name, description: typeof tool.description === "string" ? tool.description : "" }] : [])
            : [];
    }
    finally {
...
export function mcpStdioConfigs(integrations, options = {}) {
    return (integrations ?? []).map((server) => ({
        ...
        env: {
            ...
            ...(server.allowedTools?.length ? { XINYUN_MCP_ALLOWED_TOOLS: JSON.stringify(server.allowedTools) } : {}),
        },
    }));
}
...
export function mcpEnvironment(integrations) {
    return Object.fromEntries((integrations ?? [])
