...
function vendorFor(text) {
    const value = text.toLowerCase();
    if (/claude|anthropic/.test(value))
        return "anthropic";
    if (/deepseek/.test(value))
        return "deepseek";
    if (/(?:^|[/_.-])glm(?:$|[/_.-])|zhipu|thudm/.test(value))
        return "glm";
    if (/qwen|dashscope|tongyi/.test(value))
        return "qwen";
    if (/kimi|moonshot/.test(value))
        return "kimi";
    if (/gemini|google/.test(value))
        return "google";
    if (/grok|xai/.test(value))
        return "xai";
    if (/(?:^|[/_.-])gpt(?:$|[/_.-])|(?:^|[/_.-])o[1-9](?:$|[/_.-])|openai|codex/.test(value))
        return "openai";
    return undefined;
}
...
function vendorScore(vendor, text) {
...
function rankModel(id, label = id) {
    const text = `${id} ${label}`.toLowerCase();
    if (NON_CHAT_MODEL.test(text))
        return undefined;
...
export function lowerModelInCatalog(catalog, currentModel) {
    ...
    const candidates = catalog.options
        ...
        .filter((candidate) => Boolean(candidate) && candidate.id !== currentModel && candidate.vendor === current.vendor && candidate.score < current.score)
        .sort((a, b) => b.score - a.score || a.id.localeCompare(b.id));
    return candidates[0]?.id;
}
export function modelForReasoningLevel(catalog, currentModel, level) {
