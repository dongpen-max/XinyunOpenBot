...
export function providerErrorLabel(code) {
    switch (code) {
...
export function redactProviderSecrets(value) {
    return String(value ?? "")
        ...
        .replace(/\b(api[_ -]?key|x-api-key|app[_ -]?secret|client[_ -]?secret|oauth[_ -]?token|access[_ -]?token|refresh[_ -]?token|box[_ -]?token|joinUrl)\s*[=:]\s*(?:"[^"]*"|'[^']*'|[^\s,;]+)/gi, "$1=[redacted]")
