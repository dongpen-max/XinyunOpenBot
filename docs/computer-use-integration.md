# Computer use & browser use in XinyunOpen Bot

Decision doc, 2026-08-12. How bots in XinyunOpen Bot get local computer use and
...
│     one TCC prompt, named XinyunOpen Bot          │ unix socket (private)
...
1. **Spawn from the Electron main process, never from the server/gateway
   ...
   Spawned from Electron main → the grant is XinyunOpen Bot's, users see ONE
   prompt named XinyunOpen Bot, and the bundled driver inherits it. Spawned from
...
- Ship the binary at `XinyunOpen Bot.app/Contents/Resources/cua-driver`,
