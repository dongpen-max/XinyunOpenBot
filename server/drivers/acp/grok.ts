...
const support: AcpSupport = {
  ...
  // The CLI owns its own grok.com login; a leaked API key flips billing from
  // the subscription to pay-as-you-go, so the key is only forwarded when a
  // custom base URL is configured — that combination means "bring your own gateway".
  transformEnv: (env) => {
    if (!env.XAI_BASE_URL) delete env.XAI_API_KEY;
  },

  // Bind the grok.com subscription login, or accept a custom API gateway.
  pickAuthMethod: (methods) => (methods.some((m) => m.id === "cached_token") ? "cached_token" : null),
  authFailure: "fail",
  isAuthenticated: (env) => existsSync(join(homedir(), ".grok", "auth.json")) || Boolean(env.XAI_BASE_URL && env.XAI_API_KEY),
