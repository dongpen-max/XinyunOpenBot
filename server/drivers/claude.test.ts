...
describe("ClaudeDriver.decodeConfig", () => {
  it("defaults to the claude binary with acceptEdits", () => {
    const cfg = ClaudeDriver.decodeConfig({});
    expect(cfg.cli).toBe("claude");
    expect(cfg.permissionMode).toBe("acceptEdits");
    expect(cfg.models.default).toBe("claude-sonnet-5");
    expect(cfg.models.options.length).toBeGreaterThan(0);
  });
  ...
  it("forwards per-instance custom gateway credentials ahead of process env", () => {
    const env = claudeEnvironment(
      { ANTHROPIC_BASE_URL: "https://relay.example/v1", ANTHROPIC_API_KEY: "instance-key" },
      { ANTHROPIC_BASE_URL: "https://wrong.example/v1", ANTHROPIC_API_KEY: "process-key" },
    );
    expect(env.ANTHROPIC_BASE_URL).toBe("https://relay.example/v1");
    expect(env.ANTHROPIC_API_KEY).toBe("instance-key");
  });

  it("does not leak a process API key into subscription-mode instances", () => {
    const env = claudeEnvironment({}, { ANTHROPIC_API_KEY: "process-key" });
...
posixOnly("ClaudeDriver turns (fake CLI)", () => {
  ...
  const create = async (mode?: string) => {
    ...
    instance = await ClaudeDriver.create({
      ...
      environment: {},
      enabled: true,
      config: ClaudeDriver.decodeConfig({ cli: FAKE_CLI, permissionMode: "acceptEdits" }),
    });
  ...
  it("a missing binary surfaces as spawn_error, and snapshot says unavailable", async () => {
    instance = await ClaudeDriver.create({
      ...
      environment: {},
      enabled: true,
      config: ClaudeDriver.decodeConfig({ cli: join(scratch, "does-not-exist"), permissionMode: "acceptEdits" }),
    });
    recorder = recordEvents(instance.adapter);
