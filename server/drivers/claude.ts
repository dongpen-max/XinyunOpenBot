  ...
  return {
    ...
    close() {
      for (const p of [...pending.values()]) {
        if (p.ask.kind === "question") p.finish("answer", "XinyunOpen Bot: the turn is ending — wrap up.", "shutdown");
        else p.finish("deny", "XinyunOpen Bot: the turn ended", "shutdown");
      }
      try {
        server.close();
      } catch {}
...
export function claudeEnvironment(
  instanceEnvironment: Record<string, string>,
