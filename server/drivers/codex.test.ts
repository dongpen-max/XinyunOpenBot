...
describe("CodexDriver.decodeConfig", () => {
  it("defaults to the codex binary with fullAuto off", () => {
    const cfg = CodexDriver.decodeConfig({});
    expect(cfg.cli).toBe("codex");
    expect(cfg.fullAuto).toBe(false);
    expect(cfg.models.default).toBe("gpt-5.6-sol");
...
posixOnly("CodexDriver turns (fake app-server)", () => {
  ...
  const create = async (opts: { mode?: string; fullAuto?: boolean } = {}) => {
    ...
    instance = await CodexDriver.create({
      ...
      environment: {},
      enabled: true,
      config: CodexDriver.decodeConfig({ cli: FAKE_CLI, fullAuto: opts.fullAuto ?? false }),
    });
  ...
  it("a missing binary surfaces as a failed turn, and snapshot says unavailable", async () => {
    instance = await CodexDriver.create({
      ...
      environment: {},
      enabled: true,
      config: CodexDriver.decodeConfig({ cli: join(scratch, "does-not-exist"), fullAuto: false }),
    });
    recorder = recordEvents(instance.adapter);
