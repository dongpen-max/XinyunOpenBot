...
const tomlKey = (value: string) => (/^[A-Za-z0-9_-]+$/.test(value) ? value : JSON.stringify(value));

function appendMcpOverrides(
  args: string[],
  name: string,
  config: { command: string; args: string[]; env?: Record<string, string> },
  permissionMode: SendTurnInput["permissionMode"],
) {
...
export function codexAppServerArgs(turn: SendTurnInput): string[] {
  const args: string[] = [];
  ...
  if (turn.integrations?.agents) appendMcpOverrides(args, "agents", turn.integrations.agents, turn.permissionMode);
  ...
  async create(input: DriverCreateInput<CodexConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const settle = (ok: boolean, stopReason: string | null) => {
        ...
        state.settled = true;
        for (const finish of [...asks.values()]) finish("deny", "XinyunOpen Bot: the turn ended");
      ...
      const handleServerRequest = (msg: any) => {
        ...
        const mcpTool =
          Array.isArray(mcpMeta?.tool_params_display) && typeof mcpMeta.tool_params_display[0]?.name === "string"
          ...
          isMcpElicitation
            ...
            : method === "item/fileChange/requestApproval" || method === "applyPatchApproval"
              ...
              : isQuestion
                ? "ask_user"
                : "shell";
        const sendDecision = (behavior: string) => {
          if (isMcpElicitation) {
            send({
              jsonrpc: "2.0",
              id: msg.id,
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      ...
      // Codex doesn't expose a sign-in check, but a custom base URL + API key
      // is a valid authentication path for gateway users.
      const authenticated = Boolean(input.environment?.OPENAI_BASE_URL && input.environment?.OPENAI_API_KEY);
