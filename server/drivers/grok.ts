...
export interface GrokConfig {
  ...
  agentTools: boolean;
  /** Only advertise/send the OpenAI `reasoning_effort` parameter when the
   * selected provider implements it. */
...
export const GrokDriver: ProviderDriver<GrokConfig> = {
  ...
  async create(input: DriverCreateInput<GrokConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      const { threadId } = turn;
      if (!apiKey) throw new Error(`未配置 ${providerLabel} API Key（${config.apiKeyEnv}）`);
      ...
      (async () => {
        ...
        try {
          ...
          const { text, usage } = await runOpenAICompatibleToolLoop({
            ...
            onRequest: (body) =>
              appendNative(threadId, { dir: "out", source: "openai-compatible.chat.completions", msg: nativeRequest(body) }),
            onRound: (round) =>
