...
export function posixKnownDirs(
  home = homedir(),
...
export function resolvePosixExecutable(cli: string, pathValue = augmentedPath()): string | null {
  if (/[\\/]/.test(cli)) return isExecutableFile(cli) ? cli : null;
...
function whichWin(cli: string): string | null {
  ...
  for (const dir of augmentedPath().split(delimiter)) {
    ...
    if (preferNonWindowsApps && /[\\/]WindowsApps[\\/]/i.test(hit)) continue;
    return hit;
  }
  return firstHit;
}
