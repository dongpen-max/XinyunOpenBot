...
const sdkEntry = sdkRoot
  ...
  : "@trycua/cua-driver";
const { CuaDriver } = await import(sdkEntry);
...
let toolList: CuaTool[] | null = null;
...
function failure(id: unknown, message: string) {
  result(id, { content: [{ type: "text", text: message }], isError: true });
}

async function tools(): Promise<CuaTool[]> {
  if (toolList) return toolList;
  const payload = JSON.parse(await driver.listToolsJson()) as { tools?: unknown };
  toolList = Array.isArray(payload.tools)
    ? payload.tools.filter((tool): tool is CuaTool => Boolean(tool && typeof (tool as CuaTool).name === "string"))
    : [];
  return toolList;
}

function mcpTool(tool: CuaTool) {
  return {
    name: tool.name,
...
function contentFrom(value: CuaResult) {
...
async function handle(message: any) {
  ...
  if (message.method === "tools/list") return result(message.id, { tools: (await tools()).map(mcpTool) });
  if (message.method === "tools/call") {
    const name = message.params?.name;
    if (typeof name !== "string") return failure(message.id, "local computer tool name is required");
    try {
      const value = (await driver.callTool(name, JSON.stringify(message.params?.arguments ?? {}))) as CuaResult;
