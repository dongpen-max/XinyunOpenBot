...
// XinyunOpen Bot server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "XinyunOpen Bot: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
const TOOLS = [
  {
    name: "approve",
    description: "Ask the XinyunOpen Bot user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
    ...
    const text = isQuestion
      ...
      : JSON.stringify(
          answer.behavior === "allow"
            ...
            : { behavior: "deny", message: answer.message || "Denied from XinyunOpen Bot" },
        );
